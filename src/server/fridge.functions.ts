...
export const tickSimulation = createServerFn({ method: "POST" }).handler(async () => {
  ...
  const dtHours = 0.5;
  const { data: foods } = await supabaseAdmin.from("food_items").select("*");
  if (foods) {
  ...
  await supabaseAdmin.from("control_logs").insert({
...
export const getOverview = createServerFn({ method: "GET" }).handler(async () => {
  const [foods, latest, alerts, settings, energySeries, tempSeries] = await Promise.all([
    supabaseAdmin.from("food_items").select("*").order("spoilage_pct", { ascending: false }),
    supabaseAdmin.from("sensor_readings").select("*").order("created_at", { ascending: false }).limit(1).maybeSingle(),
    supabaseAdmin.from("alerts").select("*").eq("resolved", false).order("created_at", { ascending: false }).limit(20),
    supabaseAdmin.from("system_settings").select("*").eq("id", 1).maybeSingle(),
    supabaseAdmin.from("sensor_readings").select("created_at,energy_w,compressor_on").order("created_at", { ascending: false }).limit(60),
    supabaseAdmin.from("sensor_readings").select("created_at,temperature,humidity,ammonia,co2,ethylene").order("created_at", { ascending: false }).limit(60),
  ]);
...
export const addFoodItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await supabaseAdmin.from("food_items").insert({
...
export const removeFoodItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await supabaseAdmin.from("food_items").delete().eq("id", data.id);
    return { ok: true };
  });
...
export const resolveAlert = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await supabaseAdmin.from("alerts").update({ resolved: true }).eq("id", data.id);
    return { ok: true };
  });

// ---- AI Recommendation (Lovable AI Gateway) ----
export const generateRecommendation = createServerFn({ method: "POST" }).handler(async () => {
  const { data: latest } = await supabaseAdmin.from("sensor_readings")
    .select("*").order("created_at", { ascending: false }).limit(1).maybeSingle();
  const { data: foods } = await supabaseAdmin.from("food_items").select("*");
  const { data: alerts } = await supabaseAdmin.from("alerts").select("*").eq("resolved", false).limit(5);
...
export const getRecommendations = createServerFn({ method: "GET" }).handler(async () => {
  const { data } = await supabaseAdmin.from("ai_recommendations")
...
export const getForecast = createServerFn({ method: "GET" }).handler(async () => {
  const [{ data: latest }, { data: foods }] = await Promise.all([
    supabaseAdmin.from("sensor_readings").select("*").order("created_at", { ascending: false }).limit(1).maybeSingle(),
    supabaseAdmin.from("food_items").select("*"),
  ]);
...
export const getArrheniusCurve = createServerFn({ method: "GET" }).handler(async () => {
  const { data: foods } = await supabaseAdmin.from("food_items").select("name,activation_energy_kj").limit(4);
  const temps: number[] = [];
